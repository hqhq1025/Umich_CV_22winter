...
def multiples_of_ten(start: int, stop: int) -> Tensor:
    ...
    for i in range(start, stop):
        if i%10 == 0:
            num.append(i)
    x = torch.tensor(num, dtype=torch.float64)
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return x
...
def slice_assignment_practice(x: Tensor) -> Tensor:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    x[:4,0] = torch.tensor([0,0,3,3])
    x[:4,1] = torch.tensor([1,1,4,4])
    x[:4,2] = torch.tensor([2,2,3,3])
    x[:4,3] = torch.tensor([2,2,4,4])
    x[0:2,4:6] = 2
    x[2:4,4:6] = 5
    ##########################################################################
    #                            END OF YOUR CODE                            #
